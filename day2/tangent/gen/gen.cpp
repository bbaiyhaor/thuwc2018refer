...
struct P {
	...
	bool operator<(const P&a)const {for (int i = 1; i <= K; ++i)if (std::abs(a.x[i] - x[i]) > 1e-7)return x[i] < a.x[i]; return x[K] < a.x[K];}
...
ld r01() {return (rand() % 10000) / 10000.;}
#define F(aa) aa=r01()*10-5
//scanf("%lf", &aa)
void work()
{
	P circ[13];
