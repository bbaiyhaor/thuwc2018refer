...
struct P {
	...
	bool operator==(const P&a)const {return dis2(*this, a) <= 1e-8;}
	bool operator<(const P&a)const {for (int i = 1; i <= K; ++i)if (std::abs(a.x[i] - x[i]) > 1e-6)return x[i] < a.x[i]; return x[K] < a.x[K];}
...
struct Ans {
	P p[13];
	bool operator==(const Ans&ths)const {ld sum = 0; for (int i = 1; i <= K; ++i)sum += dis2(p[i], ths.p[i]); return sum < 1e-3;}
